...
interface FocusFadeProps {
  /** Hex color of the current mood. Null = neutral state */
  moodColor: string | null;
  /** Optional additional className for layout purposes */
  className?: string;
 ...
 */
export default function FocusFade({ moodColor, className = '', children }: FocusFadeProps) {
  ...
  const prevColorRef = useRef<string>(NEUTRAL_DARK);
  const currentColorRef = useRef<string | null>(moodColor);
  ...
  const theme = moodColor ? getMoodTheme(moodColor as never) : null;
