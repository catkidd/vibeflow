...
interface MusicGridProps {
  ...
  /** Called when user clicks a track card to load it in VibePlayer */
      ...
      {tracks.map((track) => {
        ...
        return (
          <motion.div
            ...
            whileHover={{ scale: 1.03, transition: { duration: DURATION.CARD_HOVER, ease: EASE.SMOOTH } }}
            whileTap={{ scale: 0.97 }}
            role="button"
            tabIndex={0}
            aria-label={`Play ${track.name} by ${track.artists.join(', ')}`}
            aria-pressed={isActive}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') onTrackSelect(track); }}
          >
            {/* Album art */}
            ...
            <div className="px-3 py-2.5">
              ...
              <p className="text-xs text-white/45 truncate mt-0.5" title={track.artists.join(', ')}>
                {track.artists.join(', ')}
              </p>
              <div className="flex items-center justify-between mt-1.5">
                <span className="text-xs text-white/30">{formatDuration(track.duration_ms)}</span>
                {track.preview_url && (
                  <span
...
function PlayingBars({ theme }: { theme: MoodThemeToken }) {
  return (
...
function SkeletonCard({ theme }: { theme: MoodThemeToken }) {
  return (
    <div
