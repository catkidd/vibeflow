...
import { motion, useMotionValue, animate, PanInfo, type Transition } from 'framer-motion';
import { SPRING, DURATION, EASE } from '@/lib/animation';

interface LiquidSliderProps {
  /** Current integer value 1–5 */
  value: number;
  /** Callback when value changes (fires continuously during drag) */
  onChange: (value: number) => void;
  /** Mood hex color used for orb glow and track fill */
  moodColor: string;
  /** Accessible label for screen readers */
  label: string;
  /** Unique id for ARIA */
  id: string;
  /** Whether this slider is inverted (Q3: higher = less stress) */
  inverted?: boolean;
}

/** Maps drag X position to integer 1–5, clamped to track bounds */
function xToValue(x: number, trackWidth: number): number {
...
function valueToX(value: number, trackWidth: number): number {
...
export default function LiquidSlider({
  ...
  const glowIntensity = value === 5 ? `0 0 16px ${moodColor}, 0 0 32px ${moodColor}80` : 'none';
  const fillPercent = ((value - 1) / 4) * 100;

  // Pulse only at value 5
  const pulseAnimation = value === 5
    ? { scale: [1, 1.12, 1] }
    : { scale: 1 };
  const pulseTransition: Transition = value === 5
  ...
  const handlePanEnd = useCallback(
    (_e: PointerEvent, info: PanInfo) => {
      ...
      const trackRect = trackRef.current.getBoundingClientRect();
      const relativeX = info.point.x - trackRect.left;
      const snappedValue = xToValue(Math.max(0, Math.min(trackWidth, relativeX)), trackWidth);
  ...
  const handleTrackClick = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      const newValue = xToValue(Math.max(0, Math.min(trackWidth, relativeX)), trackWidth);
