...
import { miniPlayerVariants, DURATION, EASE } from '@/lib/animation';
import type { MoodThemeToken } from '@/lib/moodTheme';
import type { TrackResult } from './MusicGrid';

type PlayerState = 'idle' | 'loading' | 'playing' | 'paused' | 'ended' | 'error';
type PlayerSize = 'expanded' | 'collapsed';
 ...
 */
export default function VibePlayer({ track, theme, onTrackEnd }: VibePlayerProps) {
  const [size, setSize]           = useState<PlayerSize>('expanded');
    ...
    >
      ...
      <div className="relative w-full h-32">
        {track.album.image ? (
          ...
          <div className="w-full h-full" style={{ background: theme.color20 }} />
        )}
        ...
        <div
          ...
          style={{ background: 'linear-gradient(to top, rgba(10,8,24,0.9) 0%, transparent 50%)' }}
          aria-hidden="true"
        />
      ...
      <div className="px-4 pt-3 pb-1">
        ...
        <p className="text-xs text-white/50 truncate">{track.artists.join(', ')}</p>
      </div>
      ...
      {track.preview_url && (
        <div className="px-4 pb-4">
          ...
          {/* Preview badge */}
          <p className="text-center text-xs text-white/20 mt-2">30s preview · Spotify</p>
        </div>
      )}
    </motion.div>
  );
...
function MiniEqualizer({ theme }: { theme: MoodThemeToken }) {
  return (
