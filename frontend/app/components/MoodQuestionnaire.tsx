...
import { scoreToMoodCategory, getMoodTheme, MoodCategory } from '@/lib/moodTheme';
import {
...
export interface QuestionnaireAnswers {
  q1: number; // Energy level
  q2: number; // Overall outlook
  q3: number; // Stress (INVERTED: 5 = no stress)
  q4: number; // Motivation
  q5: number; // Music preference
}

interface MoodQuestionnaireProps {
  /** Called whenever the partial score produces a new mood color */
  onMoodColorChange: (color: string) => void;
  /** Called when the user submits all 5 answers */
  onSubmit: (answers: QuestionnaireAnswers, mood: MoodCategory, color: string) => void;
...
const QUESTIONS = [
  ...
  { id: 'q4', label: 'How motivated are you to get work done?', inverted: false },
  { id: 'q5', label: 'What type of music appeals to you right now?', inverted: false },
] as const;
 ...
 */
function calculateScore(a: QuestionnaireAnswers) {
  const energyScore = a.q1 * 0.30 + a.q5 * 0.20;
  const moodScore = a.q2 * 0.30 + a.q3 * 0.20;
...
export default function MoodQuestionnaire({
  ...
  const handleSliderChange = useCallback(
    (question: keyof QuestionnaireAnswers, value: number) => {
      setAnswers((prev) => {
  ...
  return (
    <div className="w-full space-y-6">
        ...
        {QUESTIONS.map((q, i) => (
          ...
          >
            <LiquidSlider
              id={`slider-${q.id}`}
              label={`${i + 1}. ${q.label}`}
              value={answers[q.id as keyof QuestionnaireAnswers]}
              onChange={(val) =>
                handleSliderChange(q.id as keyof QuestionnaireAnswers, val)
              }
      ...
      <motion.button
        ...
        whileHover={!isSubmitting ? { scale: 1.02, boxShadow: theme.glowShadow } : {}}
