...
import { dashboardContainerVariants, dashboardExpandVariants, EASE } from '@/lib/animation';
...
const MOOD_LABELS: Record<string, string> = {
  happy: 'Happy', energetic: 'Energetic', calm: 'Calm',
  stressed: 'Stressed', sad: 'Sad', focused: 'Focused',
};
...
export default function AnalyticsPage() {
  ...
  const [moodDist, setMoodDist]   = useState<MoodSummary[]>([]);
  ...
  const avgMinPerDay  = sessions.length > 0
    ? Math.round(totalMinutes / 30)
    : 0;
  const peakDay = chartData.reduce((a, b) => a.minutes > b.minutes ? a : b, chartData[0] ?? { label: '—', minutes: 0 });
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      const sessData = await sessRes.json() as { daily_summary: DailySession[] };
      const streakData = await streakRes.json() as { streak: StreakData };
      ...
      (sessData.daily_summary ?? []).forEach((s) => {
        if (s.avg_mood) {
          moodCounts[s.avg_mood] = (moodCounts[s.avg_mood] ?? 0) + s.session_count;
        }
      });
      const total = Object.values(moodCounts).reduce((a, b) => a + b, 0) || 1;
      const dist: MoodSummary[] = Object.entries(moodCounts).map(([m, count]) => ({
        mood: m,
        count,
        ...
        >
          ...
          >
            {[
              ...
              { label: 'Daily average', value: avgMinPerDay, suffix: 'min/day' },
              { label: 'Current streak', value: streak?.current_streak ?? 0, suffix: 'd' },
              { label: 'Best streak', value: streak?.longest_streak ?? 0, suffix: 'd' },
              { label: 'Peak day', value: peakDay?.minutes ?? 0, suffix: 'min' },
            ].map((kpi) => (
              <KPICard key={kpi.label} theme={theme} loading={loading} {...kpi} />
            ))}
          </motion.div>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                  ...
                  <Area
                    ...
                    dot={false}
                    activeDot={{ r: 4, fill: theme.color, stroke: 'rgba(10,8,24,0.8)', strokeWidth: 2 }}
                  />
                </AreaChart>
              </ResponsiveContainer>
            )}
          ...
          <div className="grid gap-5" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))' }}>
            ...
            >
              ...
              {loading ? (
                ...
                <EmptyChart label="Log moods daily to see your patterns" height={160} />
              ) : (
                <>
                  <ResponsiveContainer width="100%" height={160}>
                    <RadialBarChart
                      ...
                      innerRadius="20%"
                      outerRadius="90%"
                      data={moodDist.map((m) => ({ name: MOOD_LABELS[m.mood] ?? m.mood, value: m.percentage, fill: m.color }))}
                      startAngle={90}
                      endAngle={-270}
                    >
                      ...
                      <Tooltip
                        content={({ active, payload }) => {
                          ...
                          return (
                            <div
                              className="px-3 py-2 rounded-xl text-xs"
                              style={{ background: 'rgba(10,8,24,0.95)', border: `1px solid ${d.payload.fill}55` }}
                            >
                              <span style={{ color: d.payload.fill }}>{d.payload.name}: </span>
            ...
            >
              ...
              <SessionHeatmap sessions={sessions} theme={theme} loading={loading} />
            </motion.section>
          </div>
...
/** KPI metric tile */
function KPICard({
  label, value, suffix, theme, loading,
}: {
  label: string; value: number; suffix: string;
  theme: ReturnType<typeof getMoodTheme>; loading: boolean;
}) {
  return (
    <motion.div
...
function StreakInsight({
  ...
  if (!streak) return <EmptyChart label="Start logging daily to build a streak!" height={80} />;
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-end gap-6">
        <div className="flex flex-col items-center gap-1">
          <span className="text-4xl">
            {current >= 30 ? '🏆' : current >= 14 ? '🔥🔥' : current >= 7 ? '🔥' : current >= 3 ? '🌱' : '✨'}
          </span>
          <span
        ...
        <div className="flex-1">
          ...
          <div className="h-2 rounded-full bg-white/10 overflow-hidden">
            <motion.div
              ...
              style={{ background: `linear-gradient(90deg, ${theme.color}, ${theme.complementary ?? theme.color})` }}
            />
          </div>
          ...
          <div className="flex gap-2 mt-3 flex-wrap">
            {milestones.map((m) => (
              <span
                ...
                style={{
                  background: current >= m.days ? theme.color20 : 'rgba(255,255,255,0.05)',
                  color: current >= m.days ? theme.color : 'rgba(255,255,255,0.25)',
      ...
      {nextMilestone && current < 30 && (
        <p className="text-xs text-white/35">
          🎯 {nextMilestone.days - current} more day{nextMilestone.days - current !== 1 ? 's' : ''} to earn
          {' '}<span style={{ color: theme.color }}>{nextMilestone.emoji} {nextMilestone.label}</span>
        </p>
      )}
      ...
      {streak.dominant_mood && (
        <p className="text-xs text-white/35">
          Most common mood:{' '}
          <span className="font-medium" style={{ color: MOOD_COLORS[streak.dominant_mood] ?? theme.color }}>
...
function ChartSkeleton({ theme, height = 200 }: { theme: ReturnType<typeof getMoodTheme>; height?: number }) {
  return (
    <motion.div
