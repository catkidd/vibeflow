...
import { createSpotifyApi, SPOTIFY_SCOPES } from '../config/spotify';
 ...
 */
router.get('/login', requireAuth, (req: Request, res: Response): void => {
...
router.get('/callback', requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  const storedState = getSession(req)['spotify_oauth_state'] as string;
  ...
  try {
    ...
    console.log(`[Spotify Auth] User connected. Token expires in ${expires_in}s`);
    ...
    res.redirect(`${frontendUrl}/dashboard?spotify_error=token_exchange_failed`);
  }
});
...
router.post('/refresh', requireAuth, async (req: Request, res: Response): Promise<void> => {
  const sess = getSession(req);
  const refreshToken = sess['spotify_refresh_token'] as string | undefined;

  if (!refreshToken) {
    res.status(401).json({ error: 'No Spotify refresh token. Please reconnect Spotify.' });
    return;
  }
  ...
  try {
    ...
    res.status(401).json({ error: 'Failed to refresh Spotify token. Please reconnect.' });
  }
});
 ...
 */
router.get('/token', requireAuth, async (req: Request, res: Response): Promise<void> => {
  const sess = getSession(req);
  const accessToken   = sess['spotify_access_token'] as string | undefined;
  const refreshToken  = sess['spotify_refresh_token'] as string | undefined;
  const expiresAt     = sess['spotify_token_expires'] as number | undefined;
  ...
  res.json({ connected: true, access_token: accessToken, expires_at: expiresAt });
});
 ...
 */
router.post('/disconnect', requireAuth, (req: Request, res: Response): void => {
  const sess = getSession(req);
