...
const moodCategorySchema = z.enum([
  'happy', 'calm', 'stressed', 'sad', 'energetic', 'focused',
]);
...
router.post('/', async (req: Request, res: Response): Promise<void> => {
  ...
  const sentiment_score =
    data.source === 'text' ? data.sentiment_score : null;
  ...
  try {
    const result = await pool.query(
      ...
      [userId, data.source, data.category, sentiment_score, authoritative_color],
    );
    ...
    pool.query(
      ...
      console.warn('[Streak] Non-critical streak update failed:', err.message);
    });
