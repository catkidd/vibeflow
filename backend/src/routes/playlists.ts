...
function getSession(req: import('express').Request): Record<string, unknown> { return req.session as any; }
...
interface MoodProfile {
  features: AudioFeatureTarget;
  seed_genres: string[];
  reason: string;
}

const MOOD_PROFILES: Record<MoodCategory, MoodProfile> = {
  happy: {
    features: {
      min_valence: 0.70,
      target_valence: 0.85,
...
const querySchema = z.object({
  mood: z.enum(['happy', 'energetic', 'calm', 'stressed', 'sad', 'focused']).default('focused'),
...
router.get('/', async (req: Request, res: Response): Promise<void> => {
  ...
  if (!parsed.success) {
    res.status(422).json({ error: 'Invalid query params.', fields: parsed.error.flatten().fieldErrors });
    return;
  }
  ...
  try {
    ...
    res.status(502).json({ error: 'Failed to fetch tracks from Spotify. Try again shortly.' });
  }
});
