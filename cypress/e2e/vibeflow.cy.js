...
describe("VibeFlow — Full User Journey", () => {
  ...
  describe("Mood Entry Flow (with stubbed auth)", () => {
    ...
    it("renders the mood questionnaire on /mood", () => {
      cy.visit(`${BASE}/mood`);
      cy.wait("@authMe");
      cy.get('form, [role="form"], [data-testid="questionnaire"]').should(
        "exist",
      );
    });
    ...
    it("persists selected mood to localStorage on submit", () => {
      ...
      cy.get('[id*="mood"], input[type="radio"], button[aria-label*="Happy"]')
        .first()
        .click({ force: true });
      cy.get('button[type="submit"], button')
        .contains(/submit|done|save|continue/i)
        .click({ force: true });
      cy.window().then((win) => {
  ...
  describe("Dashboard (with stubbed auth)", () => {
    beforeEach(() => {
      ...
      cy.window().then((win) =>
        win.localStorage.setItem("vf_active_mood", "focused"),
      );
    });
    ...
    it("shows the Spotify connect CTA when not connected", () => {
      ...
      cy.get("#spotify-connect-cta, #spotify-connect-btn").should("exist");
    });
